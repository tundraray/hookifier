//+------------------------------------------------------------------+
//| Webhook Client - Модуль отправки данных на вебхук                |
//| Отвечает за отправку JSON данных, проверку соединения,           |
//| обработку ошибок и повторные попытки отправки                    |
//+------------------------------------------------------------------+

//+------------------------------------------------------------------+
//| Отправка JSON данных на вебхук                                   |
//| jsonData - JSON строка для отправки                              |
//| Выполняет валидацию JSON, отправку через WebRequest,             |
//| обработку различных кодов ответа и детальное логирование ошибок  |
...
//+------------------------------------------------------------------+
//| Отправка JSON с автоматическими повторными попытками             |
//| jsonData - JSON строка для отправки                              |
//| Делает до MaxRetries попыток отправки с задержкой RetryDelay     |
//| между попытками. Использует exponential backoff можно добавить   |
//| Возвращает: true при успешной отправке, false если все неудачны  |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Проверка корректности URL вебхука                                |
//| Проверяет что URL начинается с http:// или https://              |
//| Выводит предупреждение если URL некорректный                     |
//| Используется при инициализации советника                         |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Тестирование соединения с вебхук сервером                        |
//| Отправляет тестовое сообщение при запуске советника              |
//| Содержит информацию о счете, символе, времени и версиях          |
//| Помогает проверить настройки и доступность сервера               |
//+------------------------------------------------------------------+
